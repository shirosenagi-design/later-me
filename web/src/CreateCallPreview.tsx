...
type Props = { initialDate?: Date | null; mode: 'create' | 'change'; language: ProfileLanguage; busy: boolean; error: string; onCancel: () => void; onSubmit: (value: ReservationInput) => Promise<void> }
...
const copy = {
  en: {
    ...
    maximum: 'Up to 10 years from now',
    oneAtATime: 'You can reserve one future call at a time.',
    rangeError: (minimumMinutes: number, short: boolean) => `Choose a time at least ${short ? `${minimumMinutes} minutes` : '4 hours'} from now and within 10 years.`,
  ...
  ja: {
    ...
    rangeError: (minimumMinutes: number, short: boolean) => `今から${short ? `${minimumMinutes}分以上` : '4時間以上'}、10年以内の時間を選んでください。`,
...
export default function CreateCallPreview({ initialDate, mode, language, busy, error, onCancel, onSubmit }: Props) {
  ...
  const [date, setDate] = useState(dateValue(seed))
  const [time, setTime] = useState(timeValue(seed))
  ...
  useEffect(() => {
    ...
    void fetch('/api/health', { headers: { Accept: 'application/json' } })
      ...
      .then((health) => {
        if (!active || health.dev_short_horizon_enabled !== true || health.booking_minimum_minutes !== 5) return
        setRuntime({ minimumMinutes: health.booking_minimum_minutes, developmentShortHorizon: true })
        if (mode === 'create' && !initialDate) {
          const shortMinimum = new Date(Date.now() + health.booking_minimum_minutes * 60 * 1000)
          shortMinimum.setSeconds(0, 0)
          shortMinimum.setMinutes(shortMinimum.getMinutes() + 1)
          setDate(dateValue(shortMinimum))
          setTime(timeValue(shortMinimum))
        }
      })
      .catch(() => undefined)
    return () => { active = false }
  ...
  const valid = Number.isFinite(selected.getTime()) && selected >= limits.min && selected <= limits.max

  const submit = async (event: FormEvent) => {
    event.preventDefault()
    if (valid && !busy) await onSubmit({ scheduled_for: `${date} ${time}`, future_message: message })
  }
  ...
  return <section className="form-view" aria-labelledby="future-call-title">
    ...
    <div className="form-story-grid"><form className="ocean-form" onSubmit={submit}>
      ...
      <ul className="rules"><li>{developmentShortHorizon ? text.developmentMinimum(minimumMinutes) : text.normalMinimum}</li><li>{text.maximum}</li><li>{text.oneAtATime}</li></ul>
      {!valid && <p className="range-error" role="status">{text.rangeError(minimumMinutes, developmentShortHorizon)}</p>}
      ...
      <div className="form-actions"><button className="primary-button" type="submit" disabled={!valid || busy}>{busy ? text.saving : mode === 'change' ? text.changeSubmit : text.createSubmit}</button><button className="quiet-button" type="button" onClick={onCancel}>{text.back}</button></div>
