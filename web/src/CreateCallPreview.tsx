...
export default function CreateCallPreview({ initialDate, mode, busy, error, onCancel, onSubmit, publicDemo = false }: Props) {
  ...
  return <section className="form-view" aria-labelledby="future-call-title">
    ...
    <div className="form-story-grid"><form className="ocean-form" onSubmit={submit}>
      ...
      <ul className="rules"><li>{developmentShortHorizon ? `開発ライブ確認：最短${minimumMinutes}分後から` : '最短4時間後から'}</li><li>最大10年先まで</li><li>予約できる電話は、一度に一本だけ。</li></ul>
      {!valid && <p className="range-error" role="status">今から{developmentShortHorizon ? `${minimumMinutes}分以上` : '4時間以上'}、10年以内の時間を選んでください。</p>}
